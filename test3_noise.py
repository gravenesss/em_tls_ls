...
def init():
    ...
    select_feature = ['V1/D2/F2', 'F3', 'D5/F5', 'F6', 'F9']
    ...
    for now_id in trange(seq_len, desc='Progress', unit='loop'):
        ...
        for split in range(split_num):
            ...
            for noise_id in range(noise_loop):
                ...
                # em 结果
                em_err, em_wb2 = em_fn(x2_after_std, y2_after_std, x2_with_noise_std, y2_with_noise_std,
                                       x2_with_noise_mean, y2_with_noise_mean, copy_test_x2, copy_test_y2, w_epsilon, correct)
