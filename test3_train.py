        ...
        for split in range(split_num):
            ...
            for noise_id in range(noise_loop):
                ...
                for i in range(copy_train_x2.shape[1]):  # .ravel()因为前面是(112,) 后面是(112,1) 报错：could not broadcast
                    x2_noise[:, i] = np.random.normal(loc=0, scale=x2_ratio_pattern[i] * copy_train_std_x2[i], size=(copy_train_x2.shape[0], 1)).ravel()
                y2_noise = np.random.normal(loc=0, scale=y2_ratio_pattern * copy_train_std_y2, size=(copy_train_y2.shape[0], 1))
                # 数据加入噪声
